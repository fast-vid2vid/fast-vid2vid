...
from models.I3D.models.i3d import InceptionI3d
...
def init_warping_model(checkpoint_path):
    ...
    if temporal_warping_error is None:
        logger.info("Constructing warp error module using flownet path: {checkpoint_path}..")
...
def init_i3d_model():
    ...
    if torch.cuda.is_available():
        i3d_model.cuda()
    print ('loading i3d model successfully from',i3d_model_weight)
...
def parse_args():
    ...
    parser.add_argument(
        '-go', '--global_output_file',
        type=str,
        default="../../../data/checkpoints",
        help="quantitative result log"
    )
    parser.add_argument(
...
def evaluate_video(
    ...
    result_frame_reader = FrameReader(result_video_dir, evaluate_image=True).files
    gt_frame_reader = FrameReader(gt_video_dir, evaluate_image=True).files
    if os.path.exists(video_mask_dir):
        masks = MaskReader(video_mask_dir)[:len(gt_frame_reader)]
    else:
        raise IOError("{video_mask_dir} not exists")

    # print(len(result_frame_reader))
    # print(len(gt_frame_reader))
    # print(len(masks))

    # result_frame_reader[0].save('1.png')
    # gt_frame_reader[1].save('2.png')
    # masks[2].save('3.png')    
    # exit()
...
def main(args):
    ...
    if args.only_eval_fid:
        ...
        if args.model_name is not None and args.global_output_file is not None:
            out_line = args.epoch + ', avg_mse_error:' + str(avg_mse_error)[0:6] + ' avg_ssim:' + str(avg_ssim)[0:6] + ' avg_p_dist:' + str(avg_p_dist)[0:6] + ' fid:' + str(fid_score)[0:6] + '\n'
            with open(os.path.join(args.global_output_file, args.model_name, 'quantitative.log'), 'a') as fout:
                fout.write(out_line)
