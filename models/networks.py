...
def define_G(input_nc, output_nc, prev_output_nc, ngf, which_model_netG, n_downsampling, norm, scale, gpu_ids=[], opt=[],small_G=False):
    netG = None    
    ...
    elif which_model_netG == 'singleglobal': 
        netG = SingleGlobalGenerator(opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, opt.n_blocks, opt.fg, opt.no_flow, norm_layer,small_G)

    elif which_model_netG == 'composite':
        netG = CompositeGenerator(opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, opt.n_blocks, opt.fg, opt.no_flow, norm_layer,small_G)

    elif which_model_netG == 'sr_mobile_composite':
        netG = SRMobileCompositeGenerator(opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, opt.n_blocks, opt.fg, opt.no_flow, norm_layer,small_G)
...
class BaseNetwork(nn.Module):
    ...
    def resample(self, image, flow):        
        ...
        final_grid = (self.grid.cuda(flow.get_device()) + flow).permute(0, 2, 3, 1).cuda(image.get_device())
...
class CompositeGenerator(BaseNetwork):
    def __init__(self, opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, n_blocks, use_fg_model=False, no_flow=False,
                norm_layer=nn.BatchNorm2d,small_G=False, padding_type='reflect'):
        assert(n_blocks >= 0)
    ...
    def forward(self, input, img_prev, mask, img_feat_coarse, flow_feat_coarse, img_fg_feat_coarse, use_raw_only):
        ...
        if use_raw_only:
            img_final = img_raw
            return img_final, flow, weight, img_raw, img_feat, flow_feat, None
        ...
        if use_raw_only or self.no_flow:
            ...
            flow_warp = False
            if flow_warp:
                weight_ = torch.unsqueeze(torch.clamp((((flow.mean(1)/20)+1)/2),0,1),1)#/flow.max()
                weight_ = weight_.expand_as(img_raw)
            else:
...
class SRMobileCompositeGenerator(BaseNetwork):
    def __init__(self, opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, n_blocks, use_fg_model=False, no_flow=False,
                norm_layer=nn.BatchNorm2d,small_G=False, padding_type='reflect'):
        assert(n_blocks >= 0)
        super(SRMobileCompositeGenerator, self).__init__()                
        self.opt = opt
        self.n_downsampling = n_downsampling
        self.use_fg_model = use_fg_model
        self.no_flow = no_flow
        activation = nn.ReLU6()#nn.ReLU(True)
        ResnetBlock = InvertedResidual
        if opt.dataset_mode == 'face':
            self.resize = Resize(int(opt.loadSize/2))
        elif opt.dataset_mode == 'pose':
            self.resize = Resize((256,224))
        else:
            self.resize = Resize((128,256))
        activation = nn.ReLU(True)
        ...
        if use_fg_model:
            ...
            for i in range(n_downsampling):
                s = 2 if i!=0 else 1
                mult = 2**i
                indv_down += [nn.Conv2d(ngf_indv*mult, ngf_indv*mult*2, kernel_size=3, stride=s, padding=1), 
                              norm_layer(ngf_indv*mult*2), activation]
        ...
        model_down_seg = [nn.ReflectionPad2d(3), nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0), norm_layer(ngf), activation]
        for i in range(n_downsampling):
            s = 1 if i==0 and 'teacher' not in self.opt.name else 2
            mult = 2**i
            model_down_seg += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=s, padding=1),
                               norm_layer(ngf * mult * 2), activation]  
    ...
    def forward(self, input, img_prev, mask, img_feat_coarse, flow_feat_coarse, img_fg_feat_coarse, use_raw_only):
        if 'teacher' not in self.opt.name:
            img_prev_tmp = img_prev
            input = self.resize(input)
            img_prev = self.resize(img_prev)
        else:
            img_prev_tmp = img_prev
        downsample = self.model_down_seg(input) + self.model_down_img(img_prev)
        img_feat = self.model_up_img(self.model_res_img(downsample))
        ...
        if use_raw_only or self.no_flow:
            ...
            img_warp = self.resample(img_prev_tmp[:,-3:,...].cuda(gpu_id), flow).cuda(gpu_id)   
            flow_warp = False
            if flow_warp:
                weight_ = torch.unsqueeze(torch.clamp((((flow.mean(1)/20)+1)/2),0,1),1)#/flow.max()
                weight_ = weight_.expand_as(img_raw)
...
class SingleGlobalGenerator(nn.Module):
    def __init__(self, opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, n_blocks, use_fg_model=False, no_flow=False,
                norm_layer=nn.BatchNorm2d,small_G=False, padding_type='reflect'):
        assert(n_blocks >= 0)
        super(SingleGlobalGenerator, self).__init__()                
        self.opt = opt
        self.n_downsampling = n_downsampling
        self.use_fg_model = use_fg_model
        self.no_flow = no_flow
        ...
        for i in range(n_downsampling):
            mult = 2**i
            model += [nn.Conv2d(min(ch_max, ngf * mult), min(ch_max, ngf * mult * 2), kernel_size=3, stride=2 if not ('sr' in opt.name and i==0) else 1, padding=1),
                       norm_layer(min(ch_max, ngf * mult * 2)), activation]
    ...
    def forward(self, input, img_prev, mask, img_feat_coarse, flow_feat_coarse, img_fg_feat_coarse, use_raw_only):
        in_nc = self.opt.label_nc if self.opt.label_nc!=0 else self.opt.input_nc
        input = input[:,-in_nc:]
        if 'sr' in self.opt.name:
            input = self.resize(input)
        output = self.model(input)    
        img_raw=output            
        flow= weight=  img_feat= flow_feat= img_fg_feat = None
        return output, flow, weight, img_raw, img_feat, flow_feat, img_fg_feat
...
class InvertedResidual(nn.Module):
    def __init__(self,dim, padding_type, norm_layer, activation=nn.ReLU(True), use_dropout=False,dilation=1):
        ...
        inp, oup, stride, expand_ratio = dim, dim, 1, 1
        ...
        if False:
            ...
            self.conv = nn.Sequential(
                # pw
                nn.Conv2d(inp, hidden_dim, 1, 1, 0, bias=False,dilation=dilation),
                nn.BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True),
                # dw
                nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False,dilation=dilation),
                nn.BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True),
                # pw-linear
                nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False,dilation=dilation),
                nn.BatchNorm2d(oup),
            )
...
class I3DLoss(nn.Module):
    def __init__(self):
        super(I3DLoss, self).__init__()        
        from models.I3D.evaluate import get_i3d_activations
        self.get_acts = get_i3d_activations
        self.criterion = nn.L1Loss()
        self.weights = [1.0/32, 1.0/16, 1.0/8, 1.0/4, 1.0]
        self.downsample = nn.AvgPool2d(2, stride=2, count_include_pad=False)
    ...
    def forward(self, x, y):
        #x shape: [b,f,c,h,w]
        if x.min()<0:
            x_resize = (x.clone()+1)/2
            y_resize = (y.clone()+1)/2
        else:
            x_resize,y_resize = x.clone(),y.clone()
        x_resize = F.interpolate(x_resize,size=(3,224,224))
        y_resize = F.interpolate(y_resize,size=(3,224,224))

        #x_resize = x_resize.cpu()
        #y_resize = y_resize.cpu()
        x_acts = self.get_acts(x_resize)
        y_acts = self.get_acts(y_resize)
        loss = self.criterion(x_acts,y_acts)
        #loss = loss.cuda()
        return loss
