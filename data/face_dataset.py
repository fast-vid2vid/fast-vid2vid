...
class FaceDataset(BaseDataset):
    def initialize(self, opt, is_val=False,keyframes=None):
        ...
        n_frames_total = 30
        self.dir_A = os.path.join(opt.dataroot, self.phase + '_keypoints')
        self.dir_B = os.path.join(opt.dataroot, self.phase + '_img')
        
        self.A_paths,self.B_paths = [],[]
        is_small = False
        dirs_A = sorted(glob(self.dir_A+'/*'))#[:30]
        dirs_B = sorted(glob(self.dir_B+'/*'))#[:30]

        for da,db in zip(dirs_A,dirs_B):
            #check the length of the seq
            da = sorted(glob(da+'/*'))
            db = sorted(glob(db+'/*'))
            if len(da)!= len(db) or (opt.phase=='train' and (len(da)<n_frames_total or len(db)<n_frames_total)):
                print (
                    '======do not acquire the length=======',da[0],len(da),len(db)
                )
                continue
            
            flag = False
            for i in range(len(da)):
                if da[i].split('/')[-1].split('.')[0]!=db[i].split('/')[-1].split('.')[0]:
                    flag =True
                    break
            if flag:
                continue
            self.A_paths.append(da)
    ...
    def __getitem__(self, index):
        ...
        if is_first_frame or '0000.' in A_paths[0]: # crop only the face region
        ...
        if not self.opt.isTrain and self.opt.load_sequence:
            n_frames_total = self.opt.n_frames_total
            frame_range = range(n_frames_total)
        ...
        for i in frame_range:
            ...
            points = np.loadtxt(A_paths[start_idx + i * t_step], delimiter=',')
            self.get_crop_coords(points, B_img.size)
            params = get_img_params(self.opt, self.crop(B_img).size)        
            transform_scaleA = get_transform(self.opt, params, method=Image.BILINEAR, normalize=False)
            transform_label = get_transform(self.opt, params, method=Image.NEAREST, normalize=False)
            transform_scaleB = get_transform(self.opt, params)
        ...
        if not self.opt.isTrain and not self.opt.load_sequence:
            self.A, self.B, self.I = A, B, I 
            self.frame_idx += 1 
        change_seq = False if self.opt.isTrain or (self.opt.phase=='test' and self.opt.load_sequence) or (self.opt.phase=='val' and self.opt.load_sequence)  else self.change_seq
        
        return_A_path = A_paths if (self.opt.phase=='test' and self.opt.load_sequence) or (self.opt.phase=='val' and self.opt.load_sequence) else A_path
        return_list = {'A': A, 'B': B, 'inst': I, 'A_path': return_A_path, 'change_seq': change_seq}
